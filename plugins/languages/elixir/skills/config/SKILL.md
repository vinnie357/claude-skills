...
Five worked patterns — environment variables in runtime, dev/prod split, storing `config_env()`
for runtime access, optional features, and child specs with runtime config: see
...
Moving from `use Mix.Config` to `import Config`, and moving runtime values out of
`config.exs` into `runtime.exs`: see `references/migration.md`.
