...
'Build.ExecutableOptions'` means the build script uses the pre-0.14 form.
...
zig build --watch -fincremental    # Rebuild on change, incremental (0.15+)
zig build --time-report            # Compilation performance breakdown (0.15+)
```

## Build API Methods

- `b.addExecutable()` - create executable binaries
- `b.addLibrary()` - create libraries; set `.linkage = .static` or `.dynamic` (replaces `addStaticLibrary`/`addSharedLibrary`, removed in 0.15)
- `b.addTest()` - create test suites (`.emit_object = true` for external test harnesses, 0.15+)
...
```zig
.{
    // 0.14+: name is an enum literal (valid bare Zig identifier, max 32 bytes)
    .name = .myproject,
    .version = "0.1.0",
    // 0.14+: auto-generated globally unique package id — never change it
    .fingerprint = 0x52fcd6c9b6f0b2a3, // generated by zig init
    .dependencies = .{
        .@"dep-name" = .{
            .url = "https://github.com/owner/repo/archive/refs/tags/v1.0.0.tar.gz",
            .hash = "...", // 0.14+ format embeds name-version metadata
        },
    },
}
```
