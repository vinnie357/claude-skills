...
# Supports: steps, log, failures, watch, heartbeat-status

def main [
    command: string   # Subcommand: steps, log, failures, watch, heartbeat-status
    ...
    match $command {
        ...
        "heartbeat-status" => { cmd-heartbeat-status $args }
        _ => {
            print $"(ansi red)Error:(ansi reset) Unknown command '($command)'"
            print "Available: steps, log, failures, watch, heartbeat-status"
            exit 1
        }
    }
}
...
# GET /api/runs/:run_id/steps/:step_id — show heartbeat-related fields for a step
def cmd-heartbeat-status [args: list<string>] {
    if ($args | length) < 2 {
        print $"(ansi red)Error:(ansi reset) Run ID and step ID required"
        print "Usage: debug.nu heartbeat-status <run_id> <step_id>"
        exit 1
    }
    let run_id = ($args | first)
    let step_id = ($args | get 1)
    let resp = (api-get $"/api/runs/($run_id)/steps/($step_id)")
    let step = ($resp | get data)

    # Project heartbeat-relevant fields: timing and timeout info
    # Fields shown: id, status, updated_at, timeout, timeout_extension (if present)
    let base = {
        id: ($step | get -o id)
        status: ($step | get -o status)
        updated_at: ($step | get -o updated_at)
        timeout: ($step | get -o timeout)
    }

    let with_ext = if ($step | get -o timeout_extension | is-not-empty) {
        $base | insert timeout_extension ($step | get -o timeout_extension)
    } else {
        $base
    }
