...
**Resolved (claude-skills-210):** act's `node:16-buster-slim` case used to be untrackable under the schema at all — `CHECK_METHODS` had no Docker Hub method, even though the bee's proposal named npm registry, GitHub releases, endoflife.date, and Docker Hub as the upstreams to query and only one of those four existed. `CHECK_METHODS` now also carries `npm`, `docker-hub`, and `endoflife-date` (companion identity fields `npm_package`, `docker_image`, `eol_product` — see the schema table above), closing the schema half of claude-skills-176. Creating the actual tracking entries for currently-untracked fenced literals (act's `node:16-buster-slim` itself, and others found by the sampling above) remains a content change, out of scope here per `/core:restraint` — the schema can now express them, but nobody has authored the entries yet.
