...
Worse than merely uncovered: act's `node:16-buster-slim` case **cannot be tracked at all under the current schema**, even if someone wrote the entry. `test/validate-sources.nu`'s `CHECK_METHODS` constant is `["github-releases" "hex-pm" "crates-io" "manual" "none"]` — there is no Docker Hub check method, and the bee's proposal explicitly named npm registry, GitHub releases, endoflife.date, and Docker Hub as the upstreams to query. Only one of those four exists in the schema today. Closing this half of claude-skills-176 needs a schema extension (new `check_method` values, an image-tag identity field), not just new entries — see the `## BEES REQUESTS` filed against this PR.
