...
The check also accepts every additional form node's own `semver.validRange` accepts (verified against node v24.18.0), not just the documented examples verbatim: whitespace between an operator and its version (`>= 1.2.3`), a `v`/`V` prefix (`v1.2.3`), `~>` as a tilde-range alias, and an empty or whitespace-only string as equivalent to `*` (any version). See the comment above `is-version-partial` in `scripts/validate-plugin.nu` for the full list of these decisions and why each one is deliberate rather than accidental.
