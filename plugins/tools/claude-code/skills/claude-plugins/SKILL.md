...
Run `nu <CLAUDE_SKILL_DIR>/scripts/mutation-probe.nu` on demand (not wired into `mise test` — each probe reruns the full `--self-test` suite as a subprocess) to verify `--self-test` still catches a curated set of known-dangerous mutations: neuter a check function to always-pass, or flip one of its `exit 1` branches to `exit 0`, confirm `--self-test` fails, restore the original. Add a new probe (a `find`/`replace` string pair in the script's `$probes` list) whenever a new check ships.
