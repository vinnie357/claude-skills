...
- **The verdict and each finding's disposition land as a durable record on the PR** — a PR comment, or a bees comment the PR links. Under the `approval` merge policy below, the record must be the PR comment carrying the marker line of "Merge authorization" rule 2: that precondition reads `comments` and cannot see a linked bees comment, so a bees-only record leaves it waiting. Gates 1 and 2 have observables (`mise run ci` output, `gh pr checks`); Gate 3 needs one too, or "review done" is an unverifiable assertion of exactly the kind this gate exists to catch. Check it with `gh pr view <n> --comments`.
...
9. Records are editable and deletable, and rule 2 treats the two markers asymmetrically on purpose. An edited APPROVE is discarded, which fails closed; an edited REJECT keeps blocking, because discarding it would fail open. Deleting an APPROVE likewise fails rule 2 closed, while deleting a REJECT is undetectable — GitHub documents no REST event type for comment deletion. A REJECT at a stale sha does not block, which follows from re-attestation but is asymmetric with rule 5, where a CHANGES_REQUESTED blocks whatever commit it targets. An implementer who reads records as immutable, or who applies one marker's treatment to the other, reintroduces all of this.
