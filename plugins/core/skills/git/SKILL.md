...
- **Re-read every sentence that names an edited rule or risk.** List them with `grep -noE 'rule [0-9]+|risk [0-9]+|Rule [0-9]+|Risk [0-9]+' <file>`; the sentence that described the thing goes stale, not the one edited.
