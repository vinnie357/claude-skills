...
Claude Code dynamic workflows are an optional runtime for the five-tier pipeline. When available and opted-in, the Sub-team Leader encodes one issue's pipeline as a workflow script instead of dispatching five Task spawns — the adversarial separation and stage gates (`test files unmodified before P5`, `mise run ci` green before review) become deterministic script assertions, model escalation becomes a retry ladder, and the validator↔fix iteration becomes a bounded loop. `isolation: 'worktree'` gives each parallel implementer its own worktree inside the workflow run, removed only if the agent leaves it unchanged; a Task-spawn session instead hand-creates one worktree per issue or PR per `/core:git` "Worktrees" and reuses it across review rounds. See "Worktree isolation, workflow-scoped vs session-scoped" in `references/workflows-execution.md` for the scoping.
...
- `references/dispatch-discipline.md` — Spawn-prompt rules: explicit model, specialized subagent types, lead delegates all execution, fresh worktree off `origin/main`, read-only agents never mutate the shared repository or its worktrees (scratchpad clone instead), no polling, host-inspection over claims, ADR search before proposing architecture, worktree exclusivity during review
- `references/memory-guidance.md` — Memory entries must generalize across sessions, repos, and contexts; also covers the work-vs-fact boundary, five decay checks for existing entries, and the retired `project` metadata type
