...
4. **Decomposition gate** — probe two deterministic signals (no file searching):
   ...
   `DECOMPOSITION_PATH=""` (empty string) is treated identically to unset — no log line, just proceed as A or B per bees state. If `DECOMPOSITION_PATH` is set to a non-empty value but the file is missing, emit exactly: `agent-loop: DECOMPOSITION_PATH=<value> not found; proceeding as State <B|A>`. Fall back to A (if bees has issues) or B (if empty). Do not retry, do not search, do not crash.
