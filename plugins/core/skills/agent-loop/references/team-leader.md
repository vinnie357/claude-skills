...
4. **Decomposition gate** — probe two deterministic signals (no file searching):
   ...
   `DECOMPOSITION_PATH=""` (empty string) is treated identically to unset — no log line, just proceed as A or B per bees state. If `DECOMPOSITION_PATH` is set to a non-empty value but the file is missing, emit exactly: `agent-loop: DECOMPOSITION_PATH=<value> not found; proceeding as State <B|A>`. Fall back to B (bees empty) or A (bees has issues) — matches the `<B|A>` ordering. Do not retry, do not search, do not crash.
...
6. After merge: confirm `gh pr view --json state` reports `MERGED`, then confirm the review evidence is quoted on the PR — `git worktree remove` deletes it along with the worktree. Then `git worktree remove <path>` (never `rm -rf`), then `git branch -D <branch>`, then `git pull --ff-only` in the primary checkout — see `/core:git` "Worktrees", "Cleanup, merged-only"
