...
The explicit skill list at the top is mandatory — never use `/core:*` globs in spawn prompts, since globs don't expand in Agent prompts. The mandatory core skills (per SKILL.md "Core Skills (Mandatory)") are `/core:anti-fabrication`, `/core:git`, `/core:restraint`, `/core:security`, `/core:mise`, `/core:nushell`, `/core:agent-loop`, `/core:bees`. `/core:tdd` and `/core:twelve-factor` are task-relevant here because this is code work — a Phoenix endpoint is genuinely cloud-native, test-first work — and get named explicitly alongside the mandatory stack. Domain skills (`/elixir:*` below) are added based on the issue's labels.
